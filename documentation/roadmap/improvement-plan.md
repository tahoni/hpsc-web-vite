...
**Why it matters:** A missing or misconfigured env var currently fails silently — the venue map and Contact Us captcha
just don't render, with no warning. A hardcoded `baseUrl` risks the wrong hostname shipping if the site is ever deployed
to a staging domain.
